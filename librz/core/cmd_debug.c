...
static void cmd_debug_pid(RzCore *core, const char *input) {
	...
	case 'a': // "dpa"
		if (input[2]) {
			rz_debug_attach (core->dbg, (int) rz_num_math (
						core->num, input + 2));
		} else {
...
static void rz_core_cmd_bp(RzCore *core, const char *input) {
	...
	case 'e': // "dbe"
		for (p = input + 2; *p == ' '; p++);
		if (*p == '*') rz_bp_enable_all (core->dbg->bp,true);
		else {
			for (; *p && *p != ' '; p++);
	...
	case 'd': // "dbd"
		for (p = input + 2; *p == ' '; p++);
		if (*p == '*') rz_bp_enable_all (core->dbg->bp, false);
		else {
			for (; *p && *p != ' '; p++);
...
static void rz_core_debug_esil (RzCore *core, const char *input) {
	...
	case ' ': // "de "
		{
			...
			if (p) {
				*p++ = 0;
				if (strchr (line, 'r')) perm |= RZ_PERM_R;
				if (strchr (line, 'w')) perm |= RZ_PERM_W;
				if (strchr (line, 'x')) perm |= RZ_PERM_X;
				q = strchr (p, ' ');
				if (q) {
					*q++ = 0;
					dev = p[0];
	...
	case 's': // "des"
		if (input[1] == 'u' && input[2] == ' ') { // "desu"
			...
			addr = rz_debug_reg_get (core->dbg, "PC");
			while (addr != fin) {
				rz_debug_esil_prestep (core->dbg, rz_config_get_i (
		...
		switch (input[1]) {
			...
			case ' ': // dko SIGNAL
				if (input[2]) {
					...
					//  - trace
					//  - stop
					if (signum<1) signum = rz_signal_from_string (name);
					if (signum>0) {
...
static char *get_corefile_name (const char *raw_name, int pid) {
	return (!*raw_name)?
		rz_str_newf ("core.%u", pid) :
		rz_str_trim_dup (raw_name);
}
...
#define PS(X, Y) {escaped_str = rz_str_escape (Y);rz_cons_printf(X, escaped_str);free(escaped_str);}
				if (rdi) {
	...
	case 'x': // "dx"
		...
		case ' ': { // "dx "
			...
			if (strlen (input + 2) < 4096){
				...
				if (bytes_len>0) rz_debug_execute (core->dbg,
						bytes, bytes_len, 0);
