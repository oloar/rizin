...
static bool automerge_get_predecessors_cb(void *user, const ut64 k, const void *v) {
	...
	rz_list_foreach (fcn->bbs, it, block) {
		...
		ht_up_find (ctx->visited_blocks, HT_PTR(block), &already_visited);
		if (already_visited) {
			continue;
		}
		ctx->cur_pred = block;
		...
		ht_up_insert (ctx->visited_blocks, HT_PTR(block), (void *)ctx->cur_succ_count);
	}
	return true;
}
...
RZ_API void rz_analysis_block_automerge(RzList *blocks) {
	...
	rz_list_foreach (blocks, it, block) {
		...
		rz_list_foreach (block->fcns, fit, fcn) {
			ht_up_insert (relevant_fcns, HT_PTR(fcn), NULL);
		}
	...
	rz_list_foreach_safe (blocks, it, tmp, block) {
		RzAnalysisBlock *predecessor = ht_up_find (ctx.predecessors, HT_PTR(block), NULL);
		if (!predecessor) {
			continue;
		}
		size_t pred_succs_count = (size_t)ht_up_find (ctx.visited_blocks, HT_PTR(predecessor), NULL);
		...
		for (bit = it->n; bit && (clock = bit->data, 1); bit = bit->n) {
			RzAnalysisBlock *fixup_pred = ht_up_find (ctx.predecessors, HT_PTR(clock), NULL);
			if (fixup_pred == block) {
		...
		if (rz_analysis_block_merge (predecessor, block)) { // rz_analysis_block_merge() does checks like contiguous, to that's fine
			...
			ht_up_update (ctx.visited_blocks, HT_PTR(predecessor), (void *)ctx.cur_succ_count);
			rz_list_foreach (fixup_candidates, bit, clock) {
				// Make sure all previous pointers to block now go to predecessor
				ht_up_update (ctx.predecessors, HT_PTR(clock), predecessor);
			}
